...
class PlaceDetailState extends State<PlaceDetailWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    if (isLoading) {
      ...
      final locationn = place.result.geometry.location;
      final lati = locationn.lat;
      final lngi = locationn.lng;
...
//                  cameraPosition: CameraPosition(target: center, zoom: 15.0)),
  ...
  void fetchPlaceDetail() async {
    print("PLACES3: "+"fetchPlaceDetail");
    setState(() {
      this.isLoading = true;
    ...
    print("PLACES2: "+"errorMessage"+place.errorMessage.toString());
    print("PLACES2: "+"Denied"+place.isDenied.toString());
    print("PLACES2: "+"isInvalid"+place.isInvalid.toString());
    print("PLACES2: "+"isNotFound"+place.isNotFound.toString());
    print("PLACES2: "+"isOkay"+place.isOkay.toString());
    print("PLACES2: "+"hasNoResults"+place.hasNoResults.toString());
    print("PLACES2: "+"isOverQueryLimit"+place.isOverQueryLimit.toString());
    if (mounted) {
      setState(() {
        ...
        if (place.status == "OK") {
          ...
          this.errorLoading = place.errorMessage;
          print("PLACES: "+"errorMessage"+place.errorMessage.toString());
          print("PLACES: "+"Denied"+place.isDenied.toString());
          print("PLACES: "+"isInvalid"+place.isInvalid.toString());
          print("PLACES: "+"isNotFound"+place.isNotFound.toString());
          print("PLACES: "+"isOkay"+place.isOkay.toString());
          print("PLACES: "+"hasNoResults"+place.hasNoResults.toString());
          print("PLACES: "+"isOverQueryLimit"+place.isOverQueryLimit.toString());
  ...
  ListView buildPlaceDetailList(PlaceDetails placeDetail) {
    ...
    if (placeDetail.rating != null) {
      list.add(
        Padding(
            padding:
            EdgeInsets.only(top: 10.0, left: 8.0, right: 8.0, bottom: 4.0),
            child: Text(
    ...
    if (placeDetail.types?.first != null) {
      list.add(
        Padding(
            padding:
                EdgeInsets.only(top: 8.0, left: 8.0, right: 8.0, bottom: 0.0),
            child: Text(
