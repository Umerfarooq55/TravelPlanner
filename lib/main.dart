...
class HandleState extends StatefulWidget {
  @override
  _HandleStateState createState() => _HandleStateState();
}

class _HandleStateState extends State<HandleState> {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder (
      ...
      bool seen = (snapshot.data.getBool('seen') ?? false);

      if (seen) {
        return new RootScreen();
      } else {
        return new WalkthroughScreen(prefs: snapshot.data);
      }
        }
    );
  }
